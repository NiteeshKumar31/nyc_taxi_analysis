...
BEGIN
    ...
    SET @create_sql_stmt = 
        ...
        SELECT td.year,
            ...
            CASE WHEN cal.day_name IN (''Saturday'',''Sunday'') THEN ''Y'' ELSE ''N'' END AS weekend_ind,
            SUM(CASE WHEN pt.description = ''Credit card'' THEN 1 ELSE 0 END) AS card_trip_count,
            SUM(CASE WHEN pt.description = ''Cash'' THEN 1 ELSE 0 END) AS cash_trip_count,
            SUM(CASE WHEN tt.vendor_name = ''Street-hail'' THEN 1 ELSE 0 END) AS street_hail_trip_count,
            SUM(CASE WHEN tt.vendor_name = ''Dispatch'' THEN 1 ELSE 0 END) AS dispatch_trip_count ,
        ...
        JOIN SILVER.trip_type tt ON (td.vendor_id = tt.vendor_id)
        WHERE td.year = ''' + @year + '''
          AND td.month = ''' + @month + '''
        GROUP BY td.year,
                td.month,
